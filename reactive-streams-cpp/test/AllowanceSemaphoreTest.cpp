...
TEST(AllowanceSemaphoreTest, Finite) {
  ...
  ASSERT_EQ(0, sem.release(1));
  ASSERT_FALSE(sem.isInfinite());
  ASSERT_FALSE(sem.canAcquire(2));
  ASSERT_TRUE(sem.canAcquire());
  ASSERT_TRUE(sem.tryAcquire());

  ASSERT_EQ(0, sem.release(2));
  ASSERT_EQ(2, sem.release(1));
  ASSERT_EQ(3, sem.drain());
  ASSERT_EQ(0, sem.drain());
...
TEST(AllowanceSemaphoreTest, Infinite) {
  ...
  ASSERT_EQ(0, sem.release(overHalfOfInfty));
  ASSERT_FALSE(sem.isInfinite());
...
TEST(AllowanceSemaphoreTest, DrainWithLimit) {
  ...
  ASSERT_EQ(0, sem.release(9));
  ASSERT_EQ(4, sem.drainWithLimit(4));
  ASSERT_EQ(1, sem.drainWithLimit(1));
  ASSERT_EQ(4, sem.drainWithLimit(100));
}
