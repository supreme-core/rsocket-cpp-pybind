...
static void BM_Baseline_TCP_Latency(benchmark::State& state) {
  ...
  std::thread t([&]() {
    ...
    enable = 1;
    if (setsockopt(
            serverSock, SOL_SOCKET, SO_REUSEPORT, &enable, sizeof(enable)) <
        0) {
    ...
    while (running) {
      ...
      if (recved < 0 && running) // may end while blocked on recv, so ignore
...
BENCHMARK(BM_Baseline_TCP_Latency)->Arg(32)->Arg(128)->Arg(4096);
