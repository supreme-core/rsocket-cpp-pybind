...
class MockFlowable : public Flowable<T> {
 public:
  MOCK_METHOD1_T(subscribe_, void(yarpl::Reference<Subscriber<T>>
      subscriber));
...
class MockSubscriber : public Subscriber<T> {
 public:
  MOCK_METHOD1(onSubscribe_, void(yarpl::Reference<Subscription>
      subscription));

  MOCK_METHOD1_T(onNext_, void(
      const T &value));

  MOCK_METHOD0(onComplete_, void());

  MOCK_METHOD1_T(onError_, void(folly::exception_wrapper
      ex));
...
class MockSubscription : public Subscription {
 public:
  MOCK_METHOD1(request_, void(int64_t
      n));
