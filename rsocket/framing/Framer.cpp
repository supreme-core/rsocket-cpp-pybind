...
void Framer::parseFrames() {
  ...
  while (!payloadQueue_.empty()) {
    ...
    const auto payloadSize = frameSizeWithoutLengthField(nextFrameSize);
...
StreamId Framer::peekStreamId(const folly::IOBuf& frame) {
  return FrameSerializer::peekStreamId(protocolVersion_, frame).value();
}
