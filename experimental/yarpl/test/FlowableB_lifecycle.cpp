...
static auto getFlowableBwithHandler(Scheduler& scheduler) {
  class Handler {
   public:
    ...
    Handler(const Handler&) = default; // must allow copy
    Handler& operator=(Handler&&) = default;
    Handler& operator=(const Handler&) = default; // must allow copy
...
TEST(FlowableBLifecycle, DISABLED_AsStackAllocated) {
  ...
  auto f = getFlowableBwithHandler(scheduler);
...
//
//    auto ts = TestSubscriber<long>::create(std::make_unique<MySubscriber>());
//    auto f = getFlowableBwithHandlerAsUniquePtr(scheduler);
//    std::cout << "received f, now subscribe" << std::endl;
//    f.subscribe(ts->unique_subscriber());
//    std::cout << "after runHandlerFlowable" << std::endl;
//    ts->awaitTerminalEvent();
